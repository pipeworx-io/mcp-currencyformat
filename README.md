...
`tools/list` at `https://gateway.pipeworx.io/currencyformat/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/format_currency \
  -H 'Content-Type: application/json' \
  -d '{"amount":1234.5,"currency":"EUR","locale":"de-DE"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/format_currency`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
